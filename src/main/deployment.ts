...
export class DeploymentManager {
    ...
    async deploy(config: DeploymentConfig): Promise<string> {
        ...
        try {
            ...
            const gh = await (this.github as any).getGhPath() // Accessing private method via cast or I should make it public. 
            ...
            await this.ssh.execCommand(server.host, server.port, server.username, server.keyName, `mkdir -p "${remoteDir}"`)
            ...
            await this.ssh.execCommand(server.host, server.port, server.username, server.keyName, `mkdir -p deployments`)
            ...
            const result = await this.ssh.execCommand(server.host, server.port, server.username, server.keyName, deployCmd)
